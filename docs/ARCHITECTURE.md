...
RenderController owns Worker lifecycle, snapshots, file-save dialog, progress/cancel, editing/menu locks and close-after-cancel. SettingsPanel emits values; MainWindow validates and persists settings without implementing FFmpeg logic. PreviewWidget uses the same center-Y/fit convention but remains static. Export reads existing media only; no ElevenLabs request or media upload. Default local binaries are tools/ffmpeg/bin; environment FFMPEG_PATH/FFPROBE_PATH may override them. FFmpeg 9.0.1 Windows x64 is integration-tested; other versions and large projects are not yet validated.
