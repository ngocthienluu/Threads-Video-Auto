...
from PySide6.QtWidgets import (QFileDialog, QHBoxLayout, QLabel, QMainWindow, QMessageBox,
                               QProgressBar, QPushButton, QScrollArea, QSplitter, QVBoxLayout, QWidget, QTabWidget, QComboBox, QTreeWidget)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setStyleSheet((ROOT / "app/ui/styles/dark.qss").read_text(encoding="utf-8"))
        central = QWidget()
        ...
        self.menuBar().addMenu("File").addAction("Save As...", lambda: self.save_project(save_as=True))
        ...
        left_layout.addWidget(QLabel("SCENE LIBRARY"))
        self.scene_filter = QComboBox()
        self.scene_filter.addItems(["All", "Single", "Thread"])
        self.scene_filter.currentTextChanged.connect(lambda value: self.scenes.filter_scenes(value))
        left_layout.addWidget(self.scene_filter)
        ...
        self.canvas_tabs.addTab(self.preview, "Canvas")
        center_layout.addWidget(self.canvas_tabs)
        self.editor = CommentEditor()
        editor_scroll = QScrollArea()
        editor_scroll.setWidgetResizable(True)
        editor_scroll.setWidget(self.editor)
        self.canvas_tabs.addTab(editor_scroll, "Comment / Voice")
        ...
        self.inspector_hint = QLabel("Select an object on the canvas")
        self.inspector_layout.addWidget(self.inspector_hint)
        self.layers = QTreeWidget()
        self.layers.setHeaderLabels(["Layers", "Visible", "Locked"])
        self.inspector_layout.addWidget(self.layers)
        self.right_tabs.addTab(self.inspector_host, "Inspector")
        self.right_tabs.addTab(settings_scroll, "Project")
        splitter.addWidget(left)
        splitter.addWidget(center)
        splitter.addWidget(self.right_tabs)
        splitter.setSizes([280, 650, 390])
        ...
        self.timeline_layout.addWidget(QLabel("AUTO TIMELINE - narration durations define clip timing"))
        self.workspace_splitter.addWidget(self.timeline_host)
        self.workspace_splitter.setSizes([650,250])
